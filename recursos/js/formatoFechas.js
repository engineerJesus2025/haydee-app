...
const FormatoFechas = (function() {
    ...
    return {
        ...
        // ... el resto de métodos (formatoDMA, formatoFechaHora, cambiarFormatoFecha, aLocal, formatoUltimoAcceso) se mantienen igual,
        // pero usando las constantes definidas arriba (ya lo hacen implícitamente porque están en el closure).
        ...
        formatoFechaHora(fecha) {
            return this.formatear(fecha, 'DD-MM-YYYY HH:mm');
        },
        ...
        formatoUltimoAcceso(fecha) {
            ...
            const horaFormateada = this.formatear(fechaAcceso, 'h:mm a');

            if (diffDias === 0) return `Hoy a las ${horaFormateada}`;
            if (diffDias === 1) return `Ayer a las ${horaFormateada}`;
            if (diffDias <= 7) {
                return `El ${diasCompletos[fechaAcceso.getDay()]} a las ${horaFormateada}`;
            }
            return this.formatear(fechaAcceso, 'DD/MM/YYYY') + ` a las ${horaFormateada}`;
        }
    };
})();
