...
document.querySelector("#modal_solicitud_gasto").addEventListener("hide.bs.modal", () => {
    ...
    document.getElementById("presupuesto_mensual_id").value = "";
});
...
async function registrar() {
    ...
    let fecha = document.querySelector("#fecha").value,
        ...
        presupuesto_mensual = document.querySelector("#presupuesto_mensual_id").value,
    ...
    datos_consulta.append("presupuesto_mensual_id", presupuesto_mensual);
    ...
    if (respuesta && respuesta.estatus) {
        ...
        console.log("✅ buscarPresupuesto ejecutado");
        // Esto es para actualizar el presupuesto mostrado
        await consultar();         // <- Esto es lo que te faltaba para actualizar la tabla
...
function llenarTabla(fila) {
    ...
    presupuesto.textContent = (fila["presupuesto_mensual_id"]);
...
async function modificar_formulario(e) {
    ...
    document.querySelector("#presupuesto_mensual_id").value = data.presupuesto_mensual_id;

    document.querySelector("#presupuesto_total").textContent = data.monto_presupuesto || "-";
...
async function modificar(id) {
    ...
    let presupuesto_mensual = document.querySelector("#presupuesto_mensual_id").value;
let disponible = await consultarPresupuestoDisponible(presupuesto_mensual);
disponible = parseFloat(disponible); // ⬅️ Esto evita bugs
let disponible_real = parseFloat(disponible + monto_original); // ⬅️ Asegura número exacto

console.log("🧮 Monto original:", monto_original);
console.log("📝 Monto nuevo:", monto_nuevo);
console.log("💰 Disponible (sin ajustar):", disponible);
console.log("🧾 Disponible real (ajustado):", disponible_real);
console.log("Tipos →", {
  monto_nuevo: typeof monto_nuevo,
  monto_original: typeof monto_original,
  disponible: typeof disponible,
  disponible_real: typeof disponible_real
});
...
if (monto_nuevo > disponible_real) {
    ...
    datos_consulta.append("descripcion", descripcion);
    datos_consulta.append("nombre", nombre_solicitante);
    datos_consulta.append("monto", monto_nuevo);
    datos_consulta.append("estado", estado);
    datos_consulta.append("presupuesto_mensual_id", presupuesto_mensual);
    datos_consulta.append("prioridad", prioridad);
...
async function buscarPresupuesto() {
    console.log("👉 Ejecutando buscarPresupuesto"); // prueba
    ...
    const inputPresupuestoId = document.querySelector("#presupuesto_mensual_id");
    ...
    if (respuesta && respuesta.estatus) {
        spanTotal.textContent = parseFloat(respuesta.monto_presupuesto).toFixed(2);
        spanDisponible.textContent = parseFloat(respuesta.disponible).toFixed(2);
...
async function cargarMesesYAniosConPresupuesto() {
    ...
    if (respuesta && respuesta.estatus) {
        ...
        [...mesesUnicos].sort((a, b) => a - b).forEach(mes => {
            selectorMes.innerHTML += `<option value="${mes}">${nombresMeses[mes]}</option>`;
        });

        [...aniosUnicos].sort((a, b) => b - a).forEach(anio => {
            selectorAnio.innerHTML += `<option value="${anio}">${anio}</option>`;
        });
    } else {
