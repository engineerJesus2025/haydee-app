...
let tabla = document.querySelector("#tabla_anio_fiscal"); //La tabla
let boton_formulario = document.querySelector("#boton_formulario"); // el boton
let modal = new bootstrap.Modal("#modal_anio_fiscal"); // el modal
let formulario_usar = document.querySelector(`#form_anio_fiscal`); // el form
...
async function registrar() {
	...
	//Fecha de cierre al registrar es null porque no existe :v
	...
	let res_data_table = await data_table.row.add([`${estado}`,`${fecha_inicio}`,`${fecha_cierre}`,`${descripcion}`,`${acciones.outerHTML}`]).draw();
...
async function consultar() {
	...
	document.querySelector(`#tabla_anio_fiscal tbody`).textContent = null;	
...
function llenarTabla(fila) {
	...
	let cuerpo_tabla = document.querySelector(`#tabla_anio_fiscal tbody`);

	// Creamos etiquetas
	let fila_tabla = document.createElement("tr");//creamos la fila <tr></tr>

	let id_campo = fila["id_anio_fiscal"]; // guardamos el id que nos interese
	
	// creamos un td por cada columna que vamos a llenar de la tabla <td></td>
	let estado_td = document.createElement("td"),
	fecha_inicio_td = document.createElement("td"),	
	fecha_cierre_td = document.createElement("td"), 
	descripcion_td = document.createElement("td");

	// le damos el contenido de la consulta
	estado_td.textContent = fila["estado"];
	fecha_inicio_td.textContent = fila["fecha_inicio"];
	fecha_cierre_td.textContent = fila["fecha_cierre"] || "Aun sin Cerrar";
	descripcion_td.textContent = fila["descripcion"];
	...
	fila_tabla.appendChild(fecha_inicio_td);
	fila_tabla.appendChild(fecha_cierre_td);
	fila_tabla.appendChild(descripcion_td);
...
async function modificar_formulario(e) {
	...
	fecha_cierre.value = data.fecha_cierre;
	...
	boton_formulario.setAttribute("id_modificar",data.id_anio_fiscal);
...
async function modificar(id) {	
	...
	data_table.row(`#fila-${id}`).data([`${estado}`,`${fecha_inicio}`,`${fecha_cierre}`,`${descripcion}`,`${acciones.outerHTML}`]);	
...
function init_data_table() {
	return new DataTable("#tabla_anio_fiscal",{
            destroy: true,
            responsive: true,
            "scrollX": true,
            "pageLength": 10,
...
function reasignarEventos() {
	...
	$(".eliminar").on("click",function(e){
		...
		Swal.fire({
			title: "¿Estás seguro?",
			text: "¿Está seguro que desea eliminar este Año Fiscal?",
