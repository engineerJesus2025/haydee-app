...
class Presupuesto extends Conexion
{
    ...
    public function realizar_consulta($accion){
        switch ($accion) {
            ...
            case 'consultar_mes_presupuesto':
                $respuesta = $this->consultar_mes_presupuesto();
                if ($respuesta["resultado"]) {
                    return $respuesta["datos"];
                }
                else {
            ...
            case 'consultar_anio_presupuesto':
                $respuesta = $this->consultar_anio_presupuesto();
                if ($respuesta["resultado"]) {
                    return $respuesta["datos"];
                }
                else {
    ...
    private function consultar_mes_presupuesto()
    {
        $sql = "SELECT true FROM presupuesto WHERE MONTH(fecha) = :mes LIMIT 1";

        $conexion = $this->get_conex()->prepare($sql);

        $conexion->bindParam(":mes", $this->fecha);

        $result = $conexion->execute();        
        
        $datos = $conexion->fetch(PDO::FETCH_ASSOC);

        return ["resultado"=>$result,"datos"=>(isset($datos['TRUE']))?true:false];
    }

    private function consultar_anio_presupuesto()
    {
        $sql = "SELECT true FROM presupuesto WHERE YEAR(fecha) = :anio LIMIT 1";

        $conexion = $this->get_conex()->prepare($sql);

        $conexion->bindParam(":anio", $this->fecha);
