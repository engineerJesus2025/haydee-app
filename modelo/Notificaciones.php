...
class Notificaciones extends Conexion
{
    ...
    public function set_nombre_modulo($nombre_modulo)
    {
        $this->nombre_modulo = $nombre_modulo;
    }

    public function get_nombre_modulo()
    {
    ...
    public function realizar_consulta($accion){
        ...
        switch ($accion) {
            ...
            case 'notificar_administradores':
                $validaciones = $this->validarDatos('notificar_administradores');
                if(!($validaciones["estatus"])){return $validaciones;}

                $respuesta = $this->notificar_administradores();
    ...
    {        
        $sql ="INSERT INTO notificaciones(titulo, descripcion, fecha, usuario_id, activo, nombre_modulo, referencia) VALUES (:titulo, :descripcion, :fecha, :usuario_id, 0, :nombre_modulo, :referencia)";
        $conexion = $this->get_conex()->prepare($sql);
        $conexion->bindParam(":titulo", $this->titulo);
        $conexion->bindParam(":descripcion", $this->descripcion);
        $conexion->bindParam(":fecha", $this->fecha);
        $conexion->bindParam(":usuario_id", $this->usuario_id);
        $conexion->bindParam(":nombre_modulo", $this->nombre_modulo);
    ...
    private function notificar_administradores()
    {        
        $sql ="INSERT INTO notificaciones (titulo, descripcion, fecha, usuario_id, activo, nombre_modulo, referencia)
                SELECT :titulo, :descripcion, :fecha, usuarios.id_usuario, 0, :nombre_modulo, :referencia
                FROM usuarios
                WHERE usuarios.rol_id IN (1, 2)";
