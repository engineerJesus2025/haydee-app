...
class Conciliacion_bancaria extends Conexion
{
    ...
    public function verificar_meses_conciliados()
    {
        ...
        if ($result == true) {
            $this->registrar_bitacora(CONSULTAR, GESTIONAR_CONCILIACION_BANCARIA, "TODAS LAS CONCILIACIONES");
            return $datos;
        } 
        else {
            return ["estatus"=>false,"mensaje"=>"Ha ocurrido un error con la consulta"];
    ...
    {
        $sql = "SELECT * FROM conciliacion_bancaria";
    ...
    {   
        ...
        // if ($consulta == "editar" || $consulta == "eliminar") {
            if (!(isset($this->id_conciliacion))) {return ["estatus"=>false,"mensaje"=>"El ID requerido no se recibio correctamente"];}

            if (empty($this->id_conciliacion)) {return ["estatus"=>false,"mensaje"=>"El ID requerido esta vacío"];}

            if(is_numeric($this->id_conciliacion)){
                if (!($this->validarClaveForanea("conciliacion_bancaria","id_conciliacion",$this->id_conciliacion))) {
                    return ["estatus"=>false,"mensaje"=>"La Conciliacion Bancaria seleccionada no existe"];
                }
                if ($consulta == "eliminar") {return ["estatus"=>true,"mensaje"=>"OK"];}
            }
            else{return ["estatus"=>false,"mensaje"=>"El La Conciliacion Bancaria seleccionada tiene debe ser un valor numerico entero"];}
        // }
