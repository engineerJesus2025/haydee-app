...
class Gastos extends Conexion
{
    ...
    public function registrar(){
        $sql = "INSERT INTO gastos (fecha, monto, tipo_gasto, tasa_dolar, metodo_pago, referencia, imagen, descripcion_gasto, gasto_mes_id, banco_id, proveedor_id)";
        $conexion = $this->get_conex()->prepare($sql);
...
public function listar_gastos_mes() {
    $sql = "SELECT * FROM gastos_mes ORDER BY anio DESC, mes DESC";
    $conexion = $this->get_conex()->prepare($sql);
    $conexion->execute();
...
public function filtrar_por_mes($gasto_mes_id) {
    $sql = "SELECT * FROM gastos WHERE gasto_mes_id = :id";
    $conexion = $this->get_conex()->prepare($sql);
