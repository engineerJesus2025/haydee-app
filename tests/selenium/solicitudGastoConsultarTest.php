...
class solicitudGastoConsultarTest extends TestCase
{
    private $driver;
    ...
    public function testConsultarSolicitudGastoUI()
    {
        // -----------------------------------------------------------------
        ...
        $this->driver->wait(10, 500)->until(
            function () {
                return $this->driver->findElement(WebDriverBy::xpath("//h2[contains(text(), 'GESTIONAR SOLICITUD DE GASTOS')]"));
            }
        );

        // 3.3. Definimos el *selector* del elemento "Cargando..."
        $loadingSelector = WebDriverBy::xpath("//table[@id='tabla_solicitud_gasto']//h4[contains(text(), 'Cargando...')]");
        ...
        $this->assertTrue(true, "La tabla de solicitud de gastos se cargó correctamente (desapareció 'Cargando...').");
    }
}