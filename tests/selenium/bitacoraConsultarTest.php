<?php
// tests/Selenium/bitacoraConsultarTest.php
...
class bitacoraConsultarTest extends TestCase
{
    ...
    public function testConsultarBitacoraUI()
    {
        // -----------------------------------------------------------------
        ...
        $this->driver->get('http://localhost/haydee-app/?pagina=bitacora_controlador.php&accion=inicio');

        // 3.2. Esperamos a que aparezca el título H2
        $this->driver->wait(10, 500)->until(
            function () {
                return $this->driver->findElement(WebDriverBy::xpath("//h2[contains(text(), 'BITÁCORA')]"));
            }
        );

        // 3.3. Definimos el *selector* del elemento "Cargando..."
        $loadingSelector = WebDriverBy::xpath("//table[@id='tabla_bitacora']//h4[contains(text(), 'Cargando...')]");
        ...
        $this->assertTrue(true, "La tabla de bitacoras se cargó correctamente (desapareció 'Cargando...').");
    }
}