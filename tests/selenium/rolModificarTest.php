<?php
// tests/Selenium/rolModificarTest.php
...
class rolModificarTest extends TestCase
{
    private $driver;
    ...
    public function testModificarRolUI()
    {
        // -----------------------------------------------------------------
        ...
        $this->driver->wait(10, 500)->until(
            function () {
                return $this->driver->findElement(WebDriverBy::xpath("//h2[contains(text(), 'GESTIONAR ROLES')]"));
            }
        );
        ...
        $editButtonSelector = WebDriverBy::xpath("//table[@id='tabla_roles']/tbody/tr[2]//button[@title='Editar']");
        ...
        // 4.1 Esperar a que el input 'nombre' sea visible
        $nombrerolSelector = WebDriverBy::id('nombre');
        $nombrerolField = $this->driver->wait(10)->until(
            WebDriverExpectedCondition::visibilityOfElementLocated($nombrerolSelector)
        );
        ...
        $this->driver->wait(5)->until(function ($driver) use ($nombrerolSelector) {
            return $driver->findElement($nombrerolSelector)->getAttribute('value') != '';
        });
        ...
        // 5.1 Modificar nombre de rol (ID: nombre)
        $nombrerolField->click();
        $nombrerolField->clear();
        
        // Borrado seguro si clear() falla
        if ($nombrerolField->getAttribute('value') != '') {
            $nombrerolField->sendKeys(array(WebDriverKeys::CONTROL, 'a'));
            $nombrerolField->sendKeys(WebDriverKeys::BACKSPACE);
        }
        $nombrerolField->sendKeys($nuevo_nombre);
        ...
        $tableSelector = WebDriverBy::id('tabla_roles');
        ...
        $this->assertTrue(true, "Modificación de nombre de rol exitosa.");
    }
}