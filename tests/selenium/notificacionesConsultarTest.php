<?php
// tests/Selenium/notificacionesConsultarTest.php
...
class notificacionesConsultarTest extends TestCase
{
    private $driver;
    ...
    public function testConsultarNotificacionesUI()
    {
        // -----------------------------------------------------------------
        ...
        $this->driver->get('http://localhost/haydee-app/?pagina=notificaciones_controlador.php&accion=inicio');

        // 3.2. Esperamos a que aparezca el título H2
        $this->driver->wait(10, 500)->until(
            function () {
                return $this->driver->findElement(WebDriverBy::xpath("//h2[contains(text(), 'NOTIFICACIONES')]"));
            }
        );

        // 3.3. Definimos el *selector* del elemento "Cargando..."
        $loadingSelector = WebDriverBy::xpath("//table[@id='tabla_notificaciones']//h4[contains(text(), 'Cargando...')]");
        ...
        $this->assertTrue(true, "La tabla de notificacioness se cargó correctamente (desapareció 'Cargando...').");
    }
}